...
探针在所有维度上都没有复现断流:直连 SWG 40 秒、过 mitmproxy 40 秒都完整跑完。
真实流量的两轮复现给出了答案:

| 轮次 | SWG 状态 | 结果 |
|---|---|---|
| 干净 addon | 域名被 302 风险页重新封锁 | 16 个 init 只有 7 个 complete 到 relay;这 7 个 relay 和 CPA 都完整流完并带 `response.completed`,但 addon 收到的是 302 页;codex 报 `stream closed before response.completed`,重试 5 次放弃 |
| 带 swg_auto_confirm | 302 已确认解锁 | 532KB、1655 个 delta 事件的流完整到达,`terminal=True`,codex 成功退出 |

也就是说 SWG 是**在响应侧**执行封锁策略的:请求照常转发到 relay,上游照常生成,
响应回到客户端时被换成 302 风险页;已经开始流式返回的连接则在中途被掐,这就是之前看到的 25 秒 `terminal=False`。
每次被换掉的响应上游都已经完整生成过一遍,重试只是在重复烧 token。

当前结论:25 秒断流是域名被重新封锁的伴生症状,不是流式协议本身的问题。轮询、心跳、攒完再回都不需要。
方向是让域名稳定处于已确认状态,并让 addon 在遇到 302 或中途断流时先重新确认再放行重试。
